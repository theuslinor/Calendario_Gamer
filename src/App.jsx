...
export default function App() {
  ...
  const handleSelectGame = (item) => {
    if (item.data) setViewDate(parseISO(item.data));
  ...
  const proximosLancamentos = useMemo(() => {
    const hoje = new Date();
    return releases
      .filter(i => i.data && isAfter(parseISO(i.data), hoje) && i.titulo)
  ...
  const maisHypadosFuturos = useMemo(() => {
    ...
    return releases
      .filter(i => i.hype === 'altissimo' && i.data && isAfter(parseISO(i.data), hoje))
      .sort((a, b) => new Date(a.data) - new Date(b.data));
  }, [releases]);
    ...
    <div className="min-h-screen bg-[#020617] text-slate-50 font-sans selection:bg-cyan-500/30 overflow-x-hidden">
      ...
      >
        <div className="max-w-[1600px] mx-auto w-full flex flex-row items-center justify-center gap-4 md:gap-12 lg:gap-16">
            ...
            <AnimatePresence>
              {isSearchOpen && searchResults.length > 0 && (
                <motion.div 
                  ...
                  className="absolute top-full left-0 w-full mt-2 bg-[#0f172a] border border-white/10 rounded-2xl shadow-2xl overflow-hidden z-[100] backdrop-blur-2xl"
                >
                  {searchResults.map(item => {
                    const hypeSearchStyles = {
                      altissimo: "border-orange-500/20 bg-orange-500/5 text-orange-400 hover:bg-orange-500/10",
                      alto: "border-cyan-500/20 bg-cyan-500/5 text-cyan-400 hover:bg-cyan-500/10",
          ...
          <button onClick={() => setViewDate(new Date())} className="text-[8px] md:text-[9px] font-black bg-cyan-500/10 text-cyan-400 px-3 md:px-5 py-2 md:py-2.5 rounded-xl border border-cyan-500/20 active:bg-cyan-500 transition-colors uppercase tracking-widest shrink-0 shadow-lg">Hoje</button>
        </div>
      </motion.header>
          ...
          <section className="flex flex-col w-full max-w-4xl mx-auto">
            ...
            <div className={`grid ${isMobile ? 'grid-cols-5' : 'grid-cols-7'} border-l border-t border-white/5 rounded-[2rem] overflow-hidden bg-slate-900/40 shadow-2xl`}>
              {diasDoCalendario.map((dia, index) => {
                ...
                return (
                  <div key={index} onClick={() => itensDoDia.length > 0 && setDiaSelecionado(dia)} className={`min-h-[95px] md:min-h-[115px] p-1.5 md:p-3 border-r border-b border-white/5 relative cursor-pointer transition-all ${foraDoMes ? 'opacity-10' : ''} ${hoje ? 'bg-cyan-500/5 ring-1 ring-inset ring-cyan-500/50 z-10' : ''}`}>
                    ...
                    <div className="space-y-1">
                      {itensDoDia.slice(0, 3).map(item => {
                         const hypeStyles = { altissimo: "bg-orange-600 text-white border-orange-400", alto: "bg-cyan-500/20 text-cyan-400 border-cyan-500/40", normal: "bg-white/5 text-slate-400 border-transparent" };
                         return <div key={item.id} className={`px-1.5 py-0.5 rounded-[4px] text-[8px] md:text-[11px] font-black truncate border ${hypeStyles[item.hype] || hypeStyles.normal}`}>{item.titulo}</div>
                      })}
                      {itensDoDia.length > 3 && <div className="text-[7px] md:text-[10px] font-black text-cyan-500 ml-1 flex items-center gap-1 mt-1"><ChevronRightCircle size={10} /> {itensDoDia.length - 3} mais</div>}
                    </div>
                  </div>
                );
              })}
          ...
          <aside className="lg:sticky lg:top-32 flex flex-col w-full lg:max-h-[550px]">
             ...
             <div className="flex md:flex-col overflow-x-auto md:overflow-y-auto gap-2 pb-4 no-scrollbar lg:pr-2 snap-x">
                {proximosLancamentos.map(item => {
                  const hypeSidebarStyles = { altissimo: "border-orange-500/40 bg-orange-500/5 hover:border-orange-500 shadow-xl", alto: "border-cyan-500/40 bg-cyan-500/5 hover:border-cyan-500 shadow-xl", normal: "border-white/5 bg-white/5 hover:border-white/20" };
                  return (
                    <button key={item.id} onClick={() => handleSelectGame(item)} className={`min-w-[220px] md:min-w-0 w-full text-left p-4 md:p-5 rounded-[1.2rem] md:rounded-[1.8rem] border transition-all group shrink-0 snap-center ${hypeSidebarStyles[item.hype] || hypeSidebarStyles.normal}`}>
                       <div className="flex justify-between items-start mb-1">
                          <span className={`text-[8px] md:text-[10px] font-black tracking-widest ${item.hype === 'normal' ? 'text-slate-500' : 'text-cyan-500'}`}>{format(parseISO(item.data), "dd MMM")}</span>
                       ...
                       <div className="flex gap-1.5 flex-wrap">
                          {item.plataformas?.map(p => PLATFORM_ICONS[p] && <img key={p} src={PLATFORM_ICONS[p]} alt={p} className="h-3 w-3 opacity-40 grayscale group-hover:grayscale-0 transition-all" />)}
                       </div>
                    </button>
                  );
                })}
      ...
      <div className="max-w-[1600px] mx-auto px-4 md:px-8 mt-12 space-y-24 pb-20">
        <section>
          ...
          <div className="flex gap-4 md:gap-6 overflow-x-auto no-scrollbar pb-10 snap-x px-1">
             {maisHypadosFuturos.map(item => (
               <button key={item.id} onClick={() => handleSelectGame(item)} className="min-w-[280px] md:min-w-[350px] snap-start p-8 md:p-12 rounded-[2.5rem] md:rounded-[3.5rem] bg-gradient-to-br from-orange-600/20 via-orange-950/10 to-transparent border border-orange-500/20 text-left active:border-orange-500/50 shadow-2xl group transition-all">
                  <div className="bg-orange-500/20 p-3 rounded-2xl w-fit mb-6 text-orange-500"><Star size={24} fill="currentColor" /></div>
                  <h4 className="text-xl md:text-3xl font-black uppercase italic tracking-tighter leading-tight mb-4 group-hover:text-orange-400">{item.titulo}</h4>
                  <div className="flex gap-2 mb-4">
                      {item.plataformas?.map(p => PLATFORM_ICONS[p] && <img key={p} src={PLATFORM_ICONS[p]} alt={p} className="h-4 w-4 opacity-60" />)}
                  </div>
    ...
    <div className="fixed inset-0 z-[70] flex items-center justify-center p-4">
      ...
      <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="relative w-full max-w-md bg-[#0f172a] border border-white/10 rounded-[3rem] p-8 shadow-3xl">
        <div className="flex justify-between items-center mb-8">
           <h3 className="text-[10px] font-black text-cyan-500 uppercase tracking-[0.3em]">{format(dia, "dd 'de' MMMM", { locale: ptBR })}</h3>
           <button onClick={onClose} className="p-2 bg-white/5 rounded-xl text-slate-500 hover:text-white"><X size={22} /></button>
        </div>
        <div className="space-y-3 max-h-[60vh] overflow-y-auto no-scrollbar">
          {jogos.map(item => (
            <button key={item.id} onClick={() => onSelectGame(item)} className={`w-full flex items-center justify-between p-5 rounded-2xl border border-white/5 bg-white/5 transition-all hover:border-cyan-500/30 group`}>
               <div className="flex flex-col items-start gap-1 text-left">
                 <span className="font-black uppercase italic text-sm tracking-tight">{item.titulo}</span>
                 <div className="flex gap-1">{item.plataformas?.map(p => PLATFORM_ICONS[p] && <img key={p} src={PLATFORM_ICONS[p]} alt={p} className="h-2.5 w-2.5 opacity-50" />)}</div>
               </div>
               <ChevronRightCircle size={18} className="text-cyan-500" />
            </button>
          ))}
        </div>
      </motion.div>
